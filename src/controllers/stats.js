...
exports.statistics = async (req, res) => {
  ...
  try {
    ...
    if (!month) {
      ...
      const gamesInMonth = games.filter((game) => new Date(game).getMonth() === parseInt(month));
      const statsMonth = {};
...
exports.getMyScores = async (req, res, next) => {
  ...
  try {
    ...
    const firstDayOfThisMonth = new Date(date.getFullYear(), date.getMonth(), 0);
...
exports.getRankingLists = async (req, res, next) => {
  ...
  try {
    ...
    if (Date.now() < lastDayOfLastMonth.getTime()) {
      firstDayOfLastMonth = new Date(date.getFullYear(), date.getMonth() - 1, 15);
      lastDayOfLastMonth = new Date(date.getFullYear(), date.getMonth(), 1);
    }

    const [year1, year2] = [firstDayOfLastMonth.getFullYear(), lastDayOfLastMonth.getFullYear()];
    const [month1, month2] = [months[firstDayOfLastMonth.getMonth()], months[lastDayOfLastMonth.getMonth()]];
    const [day1, day2] = [firstDayOfLastMonth.getDate(), lastDayOfLastMonth.getDate()];
    const time1 = day1 + ' ' + month1 + ' ' + year1;
    const time2 = day2 + ' ' + month2 + ' ' + year2;
    ...
    resultToday.forEach((obj, i) => {
      ...
      const data = {
        fullName: obj.user.length > 0 ? obj.user[0].fullName : 'Nepoznat',
        score: obj.score,
        duration: `${minutes}:${seconds}`,
      };
      if (!obj.user[0].isGuest) {
